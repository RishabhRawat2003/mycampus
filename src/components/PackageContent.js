...
const PackageContent = () => {
    ...
    const handleNext = () => {
        if (currentIndex + CARDS_PER_VIEW < PACKAGE.length) {
    ...
    "
            ...
            <div className="w-full">
                <div className=" m-8">
                    <h2 className="text-3xl w-[90%] sm:text-4xl md:text-5xl font-semibold text-gray-500">We'll help you plan an</h2>
                    <h1 className="text-3xl w-[90%] sm:text-4xl md:text-5xl font-semibold text-gray-500 py-3">unforgettable
                        <span className="text-primarycolor"> India holiday</span></h1>

                    <h2 className="text-xl text-gray-500 w-[90%]">   Let us organize a perfect India experience for you.</h2>
                </div>
                <div className="relative w-full scroll-smooth">
                    ...
                    <div className="flex gap-9 lg:gap-5 justify-evenly overflow-hidden px-4 lg:px-0 py-5 scroll-smooth">
                        {PACKAGE.slice(currentIndex, currentIndex + CARDS_PER_VIEW).map(
                            (data, index) => (
                                <div
                                    key={index}
                                    className="bg-white shadow-md rounded-md relative overflow-visible flex-shrink-0 w-[90%] sm:w-[45%] lg:w-52"
                                >
                                    <div className="p-4 text-gray-700 h-40 lg:h-[210px] relative z-10">
                                        <div className="flex justify-between items-center">
                                            <div>
                                                <h1 className="text-lg lg:text-xl text-gray-600 font-medium">
                                                    {data.name}
                                                </h1>
                                            ...
                                            <div>
                                                <img
                                                    src={data?.image}
                                                    alt=""
                                                    className="w-6 h-6 lg:w-7 lg:h-7 object-cover"
                                                />
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="absolute -left-4 sm:-left-6 lg:-left-8 top-16 lg:top-[85px] transform z-20">
                                        <img
                                            src={data?.image}
                                            alt=""
                                            className="w-32 h-20 sm:w-40 sm:h-24 lg:w-52 lg:h-28 object-cover rounded"
                                        />
                                    </div>
                                </div>
                            )
